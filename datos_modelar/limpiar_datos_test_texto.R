...
d_or <- fread('../datos_originales/Estimar_UH2019.txt', encoding = 'UTF-8')

imagenes <- fread('../datos_originales/numero_imagenes_test.csv')
...
limpiar <- function(df = d_or) {
  ...
  for(i in 1:nrow(df)) {
    ...
    if(is.na(df$HY_metros_totales[i])) {
      ...
      tryCatch(
        ...
        error = function(e){df[i, "HY_metros_totales"] = 0 }
  ...
  data_imagenes=data_imagenes[data_imagenes$dataset=="Test",-34]
  ...
  write.csv(df_def, 'test_data_texto_limpio.csv', row.names =F, col.names = T,
            fileEncoding = 'utf-8')
